...
{
    private Func<IConsumeContext<TMessage>, Expression<Func<TInstance, bool>>> _predicateFactory;
    private Expression<Func<IConsumeContext<TMessage>, Guid>> _correlationIdSelector;

    internal Func<IMissingInstanceConfigurator<TInstance, TMessage>, IDispatcher<IConsumeContext<TMessage>>>
        MissingInstanceBehavior { get; private set; }

    // Called at dispatch time when context is available
    internal Expression<Func<TInstance, bool>> GetPredicate(IConsumeContext<TMessage> context) =>
        _predicateFactory?.Invoke(context) ?? throw new StateMachineException.CorrelationNotConfigured();

    internal Expression<Func<IConsumeContext<TMessage>, Guid>> CorrelationSelector => _correlationIdSelector ??
    ...
    internal Guid? GetCorrelationId(IConsumeContext<TMessage> context) =>
        _correlationIdSelector?.GetGetter().Invoke(context);

    public IEventConfigurator<TInstance, TMessage> CorrelationId(
        Expression<Func<IConsumeContext<TMessage>, Guid>> selector)
    {
        ...
        {
            var contextExpr = Expression.Constant(context, typeof(IConsumeContext<TMessage>));
    ...
    public IEventConfigurator<TInstance, TMessage>
        SelectId(Expression<Func<IConsumeContext<TMessage>, Guid>> selector)
    {
        _correlationIdSelector = selector;
        return this;
    }
